...
import { clearFailedLogins, loginIsBlocked, recordFailedLogin } from "@/lib/login-guard";
...
export async function loginAction(_previous: { error: string }, formData: FormData) {
  ...
  try {
    if (await loginIsBlocked()) return { error: "For mange forsøk. Prøv igjen om 15 minutter." };
